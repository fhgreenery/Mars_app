...
import com.sjzc.fh.pedometer.entity.MessageInfo;
...
public class MessageAdapter extends BaseMultiItemQuickAdapter<MessageInfo,BaseViewHolder> {
    ...
    public MessageAdapter(ArrayList<MessageInfo> dataList, Context context) {
        ...
        addItemType(MessageInfo.TYPE_TEXT, com.sjzc.fh.pedometer.R.layout.message_board_text_item);
        addItemType(MessageInfo.TYPE_AUDIO, com.sjzc.fh.pedometer.R.layout.message_board_audio_item);
    }
    @Override
    protected void convert(BaseViewHolder baseViewHolder, final MessageInfo message) {
        ...
        switch (baseViewHolder.getItemViewType()) {
            case MessageInfo.TYPE_TEXT:
                baseViewHolder.setText(com.sjzc.fh.pedometer.R.id.tv_content,message.getContent());
                if(message.piInfo != null){
                    if(!TextUtils.isEmpty(message.piInfo.getName())){
                        baseViewHolder.setText(com.sjzc.fh.pedometer.R.id.tv_name,message.piInfo.getName());
                    }else{
                        baseViewHolder.setText(com.sjzc.fh.pedometer.R.id.tv_name,message.piInfo.getUsername());
                    }
                }
                baseViewHolder.setText(com.sjzc.fh.pedometer.R.id.tv_time , message.getCreatedAt().substring(0,16));
                break;
            case MessageInfo.TYPE_AUDIO:
                ...
                if(voice[0] != null){
                    baseViewHolder.setText(com.sjzc.fh.pedometer.R.id.tv_length, voice[0].getStrLength());
                    final ImageView ivVoice = baseViewHolder.getView(com.sjzc.fh.pedometer.R.id.iv_voice);
                    RelativeLayout rlAudio = baseViewHolder.getView(com.sjzc.fh.pedometer.R.id.rl_audio);
